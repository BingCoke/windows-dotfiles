return {
	{
		...
		config = function()
			...
			local function refresh_project_picker(prompt_bufnr)
				...
				picker:refresh(
					project_finders.project_finder({}, project_utils.get_projects(project_order_by)),
					{ reset_prompt = true }
				)
			end
	...
	},
	{
		...
		config = function()
			...
			-- 4. 重点:telescope 模糊搜目录,选中直接跳进 oil 浮窗
			local function oil_telescope_dirs()
				local ok_builtin, builtin = pcall(require, "telescope.builtin")
				local ok_actions, actions = pcall(require, "telescope.actions")
				local ok_state, action_state = pcall(require, "telescope.actions.state")
				if not (ok_builtin and ok_actions and ok_state) then
					vim.notify("telescope.nvim 未安装", vim.log.levels.ERROR)
					return
				end

				builtin.find_files({
					prompt_title = "Open Directory in Oil",
					-- 需要装 fd (sharkdp/fd);没有的话换成:
			...
			vim.keymap.set("n", "<leader>od", oil_telescope_dirs, { desc = "Oil: fuzzy find directory (telescope)" })
